...
# SAMPLE remote model — DeepSeek via api.deepseek.com. The only sample for a
# REMOTE provider: it shows the `${VAR}` api_key wiring (the key itself lives in
# the OS secret store, never here). Copy to models/<id>.md, drop the leading
# "_", then `pha key --set PHA_ARCHIVIST`. A vision variant (palaeographer)
# follows commented, since one model file is one interface.
_MODEL_DEEPSEEK_SAMPLE = """---
...
def builtin_samples() -> tuple[tuple[str, str, str], ...]:
    ...
    return (
        ...
        ("models", "_sample.deepseek.md", _MODEL_DEEPSEEK_SAMPLE),
