...
def make_server(cfg: Config) -> FastMCP:
    ...
    def pha_get_page(document_id: int, page_no: int, include_image: bool = False) -> dict:
        ...
        try:
            ...
            out["edited_served"] = ({
                "editor": eff["editor"], "model": db.row_get(eff, "editor_model"),
                "text": eff["text"], "pinned": bool(db.row_get(eff, "pinned_at")),
                "reviewed": bool(db.row_get(eff, "reviewed_at")),
            } if eff is not None else None)
            out["page_palaeographer"] = (db.row_get(page, "palaeographer")
                                        or doc["palaeographer"])
            out["page_model"] = (db.row_get(page, "palaeographer_model")
                                 or doc["palaeographer_model"])
            out["pinned"] = bool(db.row_get(page, "pinned_at"))
    ...
    def pha_collection_config(document_relpath: str) -> dict:
        ...
        from pathlib import Path
        p = (cfg.dropbox / document_relpath).resolve()
        # ONE resolution for every caller: this tool, pha_collection_status and
        # the CLI all go through sidecar.resolve_stages. Hand-rolling the
        # sidecar-vs-legacy precedence here is what let the two config tools
        # disagree about a pha.yaml-configured collection.
        sel_dir, stem = (p, None) if p.is_dir() else (p.parent, p.stem)
        res = resolve_stages(cfg, sel_dir, stem=stem, sidecar_stem=stem)
        return {
            "document": document_relpath,
            "palaeographer": res["palaeographer"],
            "editor": res["editor"],
            "prompt_source": res["prompt_source"],
            "prompt": res["prompt"],
