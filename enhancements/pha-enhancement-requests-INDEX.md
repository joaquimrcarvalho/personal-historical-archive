...
live defects** (the review-during-a-scan import, the untranslated-Latin editor,
archive-pointer resolution, and the hand-over gaps G1/G3/G5/G6), **four feature
...
| id | item | status |
|---|---|---|
| **3** | [`pha-review-scope-bug-report.md`](pha-review-scope-bug-report.md) §11 | **OPEN.** `pha review` during a scan imports every page written so far; "pending" is still mtime-based |
...
1. **#3 — `pha review` during a scan (§11).** Small and data-protecting: a false
   ...
   and make "pending" mean *body differs* as well as *mtime newer*. **~½ day.**
