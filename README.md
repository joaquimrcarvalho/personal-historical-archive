...
`extraction_passes` in the front matter). The encoder file holds the **model
config + generic framing**; the collection-specific schema and few-shot
examples live **next to the source** in
**`encoder-prompt-langextract.md`** (generated by the helper), composed after
the base prompt and after `encoder.prompt.md` (detection rules). Select one
per document/collection with an **`encoder` file**:
...
dropbox/collections/letters-from-missons/encoder-prompt-langextract.md      →  schema + examples (with the source)
```
