...
eq(docSnap.ui, { textOn: true, showImg: true, leftPct: 52, mediaPct: 45 }, 'toggles, pane splitter and image share')
eq(docSnap.plain, true, 'an explicit txt/md override')
eq(snapshotView(DEFAULTS, {}).doc, null, 'no selection -> no document')
eq(snapshotView(DEFAULTS, {}).ui, { textOn: true, showImg: false, leftPct: 38, mediaPct: 45 }, 'defaults when nothing was set')
...
eq(seedUi(docSnap), { textOn: true, showImg: true, leftPct: 52, mediaPct: 45 }, 'seeded from the snapshot')
eq(seedUi(null), { textOn: true, showImg: false, leftPct: 38, mediaPct: 45 }, 'defaults without a snapshot')
eq(seedUi({ ui: { mediaPct: 70 } }).mediaPct, 70, 'a resized image share is remembered')
eq(seedUi({ ui: { mediaPct: 0 } }).mediaPct, 45, 'a nonsense share falls back to the default')
