...
echo "   Verify: ask the agent for pha_status — that tool exists only while the row is active —"
echo "   or, from a shell in the app's environment, curl \"\$DSH_WEB_URL/pha/documents\""
echo "   (JSON = active; 404 = wrong profile or no restart since the row was added; 403 = the"
echo "    instance wants its session credentials, which is not a plugin problem)"
